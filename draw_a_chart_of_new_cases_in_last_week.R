...
mod_for_new_cases_in_last_week_chart <- for_new_cases_in_last_week_chart %>% mutate(
...
comments <- tibble(Region=factor("London", levels=levels(mod_for_new_cases_in_last_week_chart$Region)), text="Offset by 6 days")


my_plot <- mod_for_new_cases_in_last_week_chart %>% draw_a_jburnish_chart(
  x = DaysSinceMinNewCasesInLastWeek, y = NewCasesInLastWeek, group = Region, y_min = min_new_cases_in_last_week, y_max = 7100,
